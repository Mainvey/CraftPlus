...
public class RegrexParser {
    ...
    public ArrayList<Object> parse(String regrex) {
        ...
        String[] items = this.split(this.trim(regrex));
        if(this.isShaped(items)) {//处理情况：工作台有序配方
            StringBuilder recipe = new StringBuilder();
            int index = 1, width = 0, height = 0;
            for(; !this.isMap(items[index]); index++) {
                height++;
                width = items[index].length();

                recipe.append(items[index]);
            }
            if(width * height != recipe.length() || recipe.length() == 0)
                throw new IllegalArgumentException(/*有序合成配方格式错误！*/"\u6709\u5e8f\u5408\u6210\u914d\u65b9\u683c\u5f0f\u9519\u8bef\uff01");

            HashMap<Character, Ingredient> map = new HashMap<>();
            for(; index < items.length; index++) {
                String[] values = items[index].split("=");
                map.put(values[0].charAt(0), Ingredient.fromStacks(this.toItemStack(values[1])));
            }

            NonNullList<Ingredient> ingredients = NonNullList.withSize(width * height, Ingredient.EMPTY);
            for(index = 0; index < recipe.length(); index++) {
                Ingredient ingredient = map.get(recipe.charAt(index));
                if(ingredient != null) {
                    ingredients.set(index, ingredient);
                }
            }

            list.add(this.toItemStack(items[0]));
            list.add(ingredients);
            list.add(width);
            list.add(height);
        } else {//处理情况：工作台无序配方、酿造台配方、铁砧配方、熔炉配方和熔炉燃料
    ...
    /**
     * 判断规则表达式是否为有序合成配方
     * @param regrexes 规则表达式
     * @return 是否为有序配方
     */
    private boolean isShaped(String[] regrexes) {
        for(String regrex : regrexes) {
            if(this.isMap(regrex))
                return true;
        }
        return false;
    }
